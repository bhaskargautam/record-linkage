...
class TestTransE(unittest.TestCase):
    ...
    def test_tensor_cora(self):
        cora = Cora()
        entity, relation, triples = cora.get_er_model(cora.trainDataA)
        transe = Tensor_TransE(entity, relation, triples, dimension=80)
        loss = transe.train()
        logger.info("Training DataSet A Complete with loss: %f", loss)
        ent_embeddings_a = transe.get_ent_embeddings()
        transe.close_tf_session()

        entity, relation, triples = cora.get_er_model(cora.trainDataB)
        transe = Tensor_TransE(entity, relation, triples, dimension=80)
        loss = transe.train()
        logger.info("Training Dataset B Complete with loss: %f", loss)
        ent_embeddings_b = transe.get_ent_embeddings()
        transe.close_tf_session()

        entity_pairs, true_pairs = cora.get_entity_pairs()
        alligned_pairs = []
        for i in range(0, len(entity_pairs)):
            distance = abs(spatial.distance.cosine(
                                ent_embeddings_a[entity_pairs[i][0]],
                                ent_embeddings_b[entity_pairs[i][1]]))
            if distance <= config.TRANSE_THRESHOLD:
                alligned_pairs.append(entity_pairs[i])
            if entity_pairs[i] in true_pairs:
